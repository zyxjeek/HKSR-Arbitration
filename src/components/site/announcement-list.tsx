          ...
          {announcements.map((announcement) => (
            <li key={announcement.id}>
              <Link
                ...
                className="surface-subtle group flex items-center gap-2 rounded-2xl px-4 py-3 text-sm text-white/85 hover:text-white"
              >
    ...
    <div className="space-y-4">
      {announcements.map((announcement) => (
        <article
          key={announcement.id}
          id={announcement.id}
          className="scroll-mt-24 rounded-2xl border border-white/8 bg-white/[0.03] p-5"
        >
